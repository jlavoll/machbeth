...
func _build_ui() -> void:
	...
	currency_header_label.custom_minimum_size = Vector2(340, 24)
	currency_header_label.fit_content = true
	currency_header_label.add_theme_font_override("normal_font", geist_font)
	currency_header_label.add_theme_font_size_override("normal_font_size", 10)
	...
	tab_bar = TabBar.new()
	tab_bar.add_theme_font_override("font", geist_font)
	tab_bar.add_theme_font_size_override("font_size", 10)
	tab_bar.add_tab("THE PIT SHOP")
	tab_bar.add_tab("BANQUO STASH")
	tab_bar.add_tab("MACK CYBERNETIC LOCKER")
	...
	items_scroll_container.add_theme_constant_override("separation", 6)
	scroll.add_child(items_scroll_container)

	# Quick Key Hint Footer
	var footer = Label.new()
	footer.text = "HOTKEYS: [H] Nano-Repair (+50 HP) | [J] Glitch Dampener (-30% Paranoia) | [I] Stash"
	footer.add_theme_font_override("font", geist_font)
...
func _refresh_currency_header() -> void:
	...
	var scrap: int = inv_mgr.player_scrap if is_instance_valid(inv_mgr) else 0

	currency_header_label.text = "[right][color=#ffcc00]CREDITS: %d C[/color]  |  [color=#00ffcc]SCRAP: %d[/color][/right]" % [credits, scrap]
...
func _refresh_items_view() -> void:
	...
	match active_tab_index:
		0: _render_pit_shop(inv_mgr)
		1: _render_banquo_stash(inv_mgr)
		2: _render_mack_locker(inv_mgr)

func _render_pit_shop(inv_mgr: UnifiedInventoryManager) -> void:
	for item_id in inv_mgr.pit_shop_inventory:
...
func _render_mack_locker(inv_mgr: UnifiedInventoryManager) -> void:
	...
	grid_btn.add_theme_font_override("font", geist_font)
	grid_btn.add_theme_font_size_override("font_size", 10)
...
func _build_item_card(item: Dictionary, context: String, inv_mgr: UnifiedInventoryManager, qty: int = 1) -> PanelContainer:
	...
	name_lbl.add_theme_font_override("font", geist_font)
	...
	sub_lbl.add_theme_font_override("font", geist_font)
	...
	if context == "SHOP":
		...
		btn_buy.text = "BUY (%d C + %d SCRAP)" % [cost_c, cost_s]
		btn_buy.add_theme_font_override("font", geist_font)
		btn_buy.add_theme_font_size_override("font_size", 9)
		...
		if item.get("category", "") == "CONSUMABLE":
			...
			btn_use.text = "USE"
			btn_use.add_theme_font_override("font", geist_font)
			btn_use.add_theme_font_size_override("font_size", 9)
		...
		btn_dismantle.text = "SCRAP (+%d)" % item.get("scrap_yield", 10)
		btn_dismantle.add_theme_font_override("font", geist_font)
		btn_dismantle.add_theme_font_size_override("font_size", 9)
		...
		var btn_dismantle = Button.new()
		btn_dismantle.text = "SCRAP (+%d)" % item.get("scrap_yield", 20)
		btn_dismantle.add_theme_font_override("font", geist_font)
		btn_dismantle.add_theme_font_size_override("font_size", 9)
		btn_dismantle.pressed.connect(func():
